...
| Migration zero-downtime (expand/contract) | Simplicidade da migration | Expand-contract obrigatório em tabela > 1M linhas ou serviço com SLA. Migration "drop column then add" simples só no ambiente de desenvolvimento ou tabela < 100k. Sempre dois deploys (expand → backfill → contract) com gap pra rollback. |
